...
function kindIcon(kind: string): { icon: LucideIcon; color: string } {
  if (kind === "sale") return { icon: IndianRupee, color: tokens.color.brand };
  if (kind === "payment" || kind === "receipt") return { icon: HandCoins, color: tokens.color.grn };
  return { icon: FileText, color: tokens.color.ink3 };
}

function ActivityCard({ row }: { row: ActivityRow }) {
  const { icon: Icon, color } = kindIcon(row.kind);
  const credit = Number(row.credit ?? 0);
  const debit = Number(row.debit ?? 0);
  const amountTxt =
    credit > 0 ? `+${moneyINR(credit)}` : debit > 0 ? `-${moneyINR(debit)}` : "—";
  const amountColor = credit > 0 ? tokens.color.grn : debit > 0 ? tokens.color.red : tokens.color.ink4;
  return (
    <View style={s.actRow}>
      <View style={[s.actChip, { backgroundColor: tokens.color.fill }]}>
        ...
        >
          <View style={s.hero}>
            ...
            <View style={s.heroActions}>
              <Pressable
                ...
                style={({ pressed }) => [s.heroBtn, { backgroundColor: tokens.color.brandWash }, pressed && { opacity: 0.85 }]}
              >
                <Navigation size={14} color={tokens.color.brand} />
                <Text style={[s.heroBtnTxt, { color: tokens.color.brand }]}>Navigate</Text>
              </Pressable>
              {store.phone ? (
                <Pressable
                  onPress={() => void Linking.openURL(`tel:${store.phone}`)}
                  accessibilityLabel="Call store"
                  style={({ pressed }) => [s.heroBtn, { backgroundColor: tokens.color.grnWash }, pressed && { opacity: 0.85 }]}
                >
                  <Phone size={14} color={tokens.color.grn} />
                  <Text style={[s.heroBtnTxt, { color: tokens.color.grn }]}>Call</Text>
                </Pressable>
              ) : null}
            </View>
          </View>
          ...
          <View style={s.quickRow}>
            {can("cashmemo.create") ? (
              ...
              >
                <Receipt size={16} color={tokens.color.grn} />
                <Text style={[s.quickTxt, { color: tokens.color.grn }]}>Record Sale</Text>
              </Pressable>
            ) : null}
            {can("receipt.record") ? (
              <Pressable
                ...
                style={({ pressed }) => [s.quickBtn, pressed && { opacity: 0.9 }]}
              >
                <HandCoins size={16} color={tokens.color.amb} />
                <Text style={[s.quickTxt, { color: tokens.color.amb }]}>Collect</Text>
              </Pressable>
            ) : null}
          </View>
...
const s = StyleSheet.create({
  ...
  chipTxt: { color: tokens.color.brand, fontFamily: tokens.font.sansBold, fontSize: tokens.size.lg },
  heroMain: { flex: 1, minWidth: 0 },
  heroName: { color: tokens.color.ink, fontFamily: tokens.font.sansSemi, fontSize: tokens.size.base },
  heroSub: { color: tokens.color.ink3, fontFamily: tokens.font.sans, fontSize: tokens.size.xs, marginTop: 1 },
  heroCode: {
    color: tokens.color.ink4, fontFamily: tokens.font.mono, fontSize: tokens.size.xs,
  ...
  actDesc: { color: tokens.color.ink3, fontFamily: tokens.font.sans, fontSize: tokens.size.eyebrow, marginTop: 1 },
  actDate: {
