...
export function EditBomForm({
  ...
  const [effectiveFrom, setEffectiveFrom] = useState(bom.effectiveFrom);
  const [effectiveTo, setEffectiveTo] = useState(bom.effectiveTo ?? "");
      ...
      <Panel title="Header">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Parent item" required>
            <Input value={`${bom.parentSku} — ${bom.parentName}`} disabled />
          </Field>
          ...
          <Field label="Effective to" htmlFor="effective_to" hint="Close the old BOM on this date">
